...
class GoogleSheet():
    ...
    def init(self, sheetName):
        try:
            ...
            sheet = client.open(sheetName).worksheet("匯率")  # 試算表宣告(括號內要改成你自己雲端裡面的sheet檔案)
    ...
    def update(self, row, lat, data):
        try:
            res = sheet.update_cell(row, lat, data)
        except:
            return "update error"
        else:
            return res
    ...
    def insert_row(self, data, row=3):
        try:
            res = sheet.insert_row(data, row)
        except:
            return "cell error"
        else:
            return res
