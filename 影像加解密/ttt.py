...
def encodeing(vis_o, use_key):  # 圖片加密
    ...
    # 加密開始
    count = 0
    for _ in range(1):  # 做幾次
        for i in range(w):  # 輪廓打亂
...
def decodeing(vis_o, use_key):  # 圖片解密
    # 解密
    for _ in range(1):  # 做幾次
        count = 31
        vis = vis.transpose(1, 0, 2)
        for i in range(h - 1, -1, -1):  # 輪廓打亂
            t = use_key[0][count]  # 混沌亂數加入
            if count <= 0:
                count = 31
            else:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if pc_flage == 2:
            ...
            vise = encodeing_colr(vise, key)

        # 加密區塊

        # 顯示區塊
        cv2.imshow(u'Original video', vis)
        cv2.imshow(u'Original video', vise)
        cv2.waitKey(10)
